...
{
    __weak TZImageManager *weakSelf = self;
    [self.assetLibrary writeImageToSavedPhotosAlbum:image.CGImage
                                     ...
                                     completionBlock:^(NSURL *assetURL, NSError *error) {
                                         ...
                                         TZImageManager  *strongSelf = weakSelf;
    ...
    [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAll
                                      usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
                                          ...
                                          if ([albumName compare: [group valueForProperty:ALAssetsGroupPropertyName]]==NSOrderedSame) {
                                              ...
                                              [strongSelf.assetLibrary assetForURL: assetURL
                                          ...
                                          if (group==nil && albumWasFound==NO) {
                                              ...
                                              __weak ALAssetsLibrary *weakLibray = strongSelf.assetLibrary;
                                              
                                              [strongSelf.assetLibrary addAssetsGroupAlbumWithName:albumName
...
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(NSArray<TZAssetModel *> *))completion {
    ...
    if ([result isKindOfClass:[PHFetchResult class]]) {
        for (PHAsset *asset in result) {
            ...
            [photoArr addObject:[TZAssetModel modelWithAsset:asset type:type timeLength:timeLength date:asset.modificationDate]];
        }
        ...
        [gruop enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                ...
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr addObject:[TZAssetModel modelWithAsset:result type:type timeLength:timeLength date:[result valueForKeyPath:ALAssetPropertyDate]]];
            } else {
                [photoArr addObject:[TZAssetModel modelWithAsset:result type:type]];
            }
        }];
